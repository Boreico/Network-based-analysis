...
```{r}
res_df500[order(res_df500$pvalue),]
```

```{r}
res_df500[order(res_df500$CHANGE),]
```

```{r}
res_df500[order(res_df500$CHANGE, decreasing = T),]
```
...
```{r}
res_df500_raw[order(res_df500_raw$pvalue),]
```

```{r}
res_df500_raw[order(res_df500_raw$CHANGE, decreasing = T),]
```
...
```{r}
res_dfall[order(res_dfall$pvalue),]
```
...
resall_names[resall_names$GENE == "ENSG00000109107", ]
```

```{r}
resall_names[order(resall_names$pvalue),]
```
